...
card6.addEventListener('click', function(){
  if(hint1.style.display == 'block'){
    alert('Correct on the first hint---NAILED IT !!!!!. Points Earned: 60');
    location.reload();
  }
  else if(hint2.style.display == 'block'){
    alert('Fantastic !!!! Be Focused. Points Earned: 50');
    location.reload();
  }
  else if(hint3.style.display=='block'){
    alert('A Football freak--Points Earned: 40');
    location.reload();
  }
  else if(hint4.style.display=='block'){
    alert('Took some time.But well done. Points Earned: 20');
    location.reload();
  }
  else if(hint5.style.display=='block'){
    alert('Correct, but almost equivalents to a loser. Points Earned: 10');
    location.reload();
  }
});
...
btn5.addEventListener('click', function(){
  alert("OOPS you couldn't  Answer 🤦‍🤦‍ TRY AGAIN BUDDY !!!!!!!!!!");
  location.reload();
});
